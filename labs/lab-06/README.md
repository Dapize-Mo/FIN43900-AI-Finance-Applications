...
**Date:** September 10, 2026 (Session 6)  
**Session Token:** `Reverse DCF`  

---

## 📌 Session 6 Key Deliverables & Quick Links

| Checkout Item | Deliverable File / Quick Link | Description |
|---|---|---|
| 🔄 **Reverse DCF Analysis** | [`pep_dcf_model_lab06.py`](pep_dcf_model_lab06.py#L225-L310) | Implements `reverse_dcf_for_growth` with bisection search. Solves target market price ($143.21) for implied 5-year FCFF CAGR (+0.33 pts / 3.73% CAGR). |
| 📊 **Sensitivity Analysis Grid** | [`pep_sensitivity_grid.csv`](pep_sensitivity_grid.csv)<br>[`pep_dcf_model_lab06.py`](pep_dcf_model_lab06.py#L145-L220) | Full 5×4 and 3×3 valuation matrices across WACC (6.0%–8.0%) and Terminal Growth $g$ (1.5%–3.0%). |
| 📋 **Lab 06 Brightspace Checkout** | [`lab-06-checkout.md`](lab-06-checkout.md) | Official checkout document containing all 5 Merit Anchors, Sensitivity Grid, Reverse DCF, Sources, Reasonableness, and Watch-Defer call. |
| 🐍 **Root DCF Engine** | [`../../dcf.py`](../../dcf.py) | Standalone single-command script executing synthetic training verification, sensitivity matrix, and reverse DCF. |
...
| [`pep_sensitivity_grid.csv`](pep_sensitivity_grid.csv) | Exported sensitivity grid of per-share values across WACC (6.0%–8.0%) and Terminal Growth (1.5%–3.0%). |
| [`lab-06-checkout.md`](lab-06-checkout.md) | Official Brightspace submission package with field-by-field answers, causal scenario tables, and growth note. |
| [`pep_inputs.md`](pep_inputs.md) | Sourced 10-K primary input table with filing locators, as-of dates, and normalization rationale. |
...
### 3. WACC / Terminal Growth Sensitivity Grid ($ per Diluted Share)
...
- **Solved Implied 5-Year FCFF Compounding Rate:** **3.73% per year** (Uniform Shift: **+0.33 percentage points**).
